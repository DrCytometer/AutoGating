# do_gate_naive_scm_cd4.r

do.gate.naive.scm.cd4 <- function( fcs.exp.data, fcs.gates,
                                   fcs.calculate, figure.dir )
{
  fcs.gate.label <- "Memory Stem CD4+ T cells"
  fcs.gate.marker <- c( "CD4", "CD95" )
  ...
  if ( fcs.calculate )
  {
    ...
    fcs.gates$naive.scm.cd4 <- list(
      population = list( parent = fcs.gate.parent,
                         boundary = fcs.gate.boundary ) )
  }

  fcs.gate.boundary <- fcs.gates$naive.scm.cd4$population$boundary
  ...
  fcs.gates$naive.scm.cd4$population$idx <-
  ...
  ggsave( file.path( figure.dir, "naive_scm_cd4.png" ),
          width = 9, height = 7.5 )
