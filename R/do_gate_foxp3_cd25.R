# do_gate_foxp3_cd25.r

do.gate.foxp3.cd25 <- function( fcs.exp.data, fcs.gates, fcs.calculate,
                                figure.dir )
{
  fcs.gate.a.label <- "Treg"
  fcs.gate.b.label <- "Non-Treg"

  fcs.gate.marker <- c( "FOXP3", "CD25" )
  ...
  if ( fcs.calculate )
  {
    ...
    fcs.gate.xthr <- gate.tail( fcs.gate.data.calc[ , 1 ], 0.99 )

    fcs.gate.a.xmin <- fcs.gate.xthr
    fcs.gate.a.xmax <- -0.01 * min( fcs.gate.data[ , 1 ] ) +
      1.01 * max( fcs.gate.data[ , 1 ] )
    fcs.gate.a.ymin <- 1.01 * min( fcs.gate.data[ , 2 ] ) +
      -0.01 * max( fcs.gate.data[ , 2 ] )
    fcs.gate.a.ymax <- -0.01 * min( fcs.gate.data[ , 2 ] ) +
      1.01 * max( fcs.gate.data[ , 2 ] )

    # note manual threshold for b.xmin
    fcs.gate.b.xmin <- as.numeric( quantile( fcs.gate.data[ , 1 ], 2e-5 ) )
    fcs.gate.b.xmax <- fcs.gate.xthr
    fcs.gate.b.ymin <- 1.01 * min( fcs.gate.data[ , 2 ] ) +
      -0.01 * max( fcs.gate.data[ , 2 ] )
    fcs.gate.b.ymax <- -0.01 * min( fcs.gate.data[ , 2 ] ) +
      1.01 * max( fcs.gate.data[ , 2 ] )
  ...
  fcs.gate.a.boundary <- fcs.gates$foxp3.cd25$treg$boundary
  fcs.gate.b.boundary <- fcs.gates$foxp3.cd25$non.treg$boundary
  ...
  fcs.gates$foxp3.cd25$treg$idx <- fcs.gate.parent.idx[fcs.gate.a.pip == 1 ]
  fcs.gates$foxp3.cd25$non.treg$idx <- fcs.gate.parent.idx[
    fcs.gate.b.pip == 1 ]
  ...
  fig.limit.x <- x.transform( c( -2e3, 265e3 ) )
  ...
  ggsave( file.path( figure.dir, "foxp3_cd25.png" ),
          width = 9, height = 7.5 )
